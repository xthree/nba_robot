...
(function (ESPNSS) {
    ...
    class Twitter {
        constructor() {
            ...
            this.oauth = new OAuth.OAuth('https://api.twitter.com/oauth/request_token', 'https://api.twitter.com/oauth/access_token', this.twitter_application_consumer_key, this.twitter_application_secret, '1.0A', null, 'HMAC-SHA1');
        }
        sendTweet(pTweetMessage) {
            var status = pTweetMessage; // This is the tweet (ie status)
            var postBody = {
                'status': status
            };
            this.oauth.post('https://api.twitter.com/1.1/statuses/update.json', this.twitter_user_access_token, // oauth_token (user access token)
